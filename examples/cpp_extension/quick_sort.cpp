...
LISP_API ValuePtr quick_sort(const std::vector<ValuePtr>& params) {
    if (params.size() != 1) {
        throw ValueError("quick_sort: expected one argument");
    }
    ...
    if (inputVec.back()->getType() != ValueType::NIL) {
        throw ValueError("quick_sort: expected a list");
    }
    inputVec.pop_back();
    auto vec = inputVec | std::views::transform([](ValuePtr v) static {
                   if (v->getType() != ValueType::NUMBER) {
                       throw ValueError("quick_sort: expected a number in the list");
                   }
                   return *v->asNumber();
               }) |
    ...
    std::ranges::sort(vec);
    const auto res =
        vec | std::views::transform([](double v) { return static_cast<ValuePtr>(LISP_NUM(v)); }) |
        std::ranges::to<std::vector>();
